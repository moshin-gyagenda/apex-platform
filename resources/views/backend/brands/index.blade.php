                ...
                <div class="bg-white rounded-lg border border-gray-200 p-4">
                    <div class="flex items-center justify-between">
                        ...
                        <div class="w-12 h-12 rounded-lg bg-primary-50 flex items-center justify-center">
                            <i data-lucide="tag" class="w-6 h-6 text-primary-500"></i>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div id="brands-content">
                    @if($brands->isEmpty())
                        <div class="flex flex-col items-center justify-center py-12">
                            ...
                            <h3 class="text-lg font-medium text-gray-700">No brands found</h3>
                            <p class="mt-2 text-sm text-gray-500 text-center max-w-sm">
                                You don't have any brands yet. Add your first brand to get started.
                            </p>
                            <a href="{{ route('admin.brands.create') }}" class="mt-4 inline-flex items-center px-4 py-2 bg-primary-500 text-white rounded-lg hover:bg-primary-600 transition-colors font-medium">
                                <i data-lucide="plus" class="w-4 h-4 mr-2"></i>
                                ...
                                <tbody>
                                    @foreach($brands as $index => $brand)
                                        <tr class="border-b border-gray-100 hover:bg-gray-50 transition-colors {{ $index % 2 === 0 ? 'bg-white' : 'bg-gray-50/30' }}">
                                            <td class="py-3 px-4 text-sm text-gray-600">{{ $brand->id }}</td>
                                            <td class="py-3 px-4">
                                                <div class="flex items-center">
                                                    ...
                                                    <div class="font-medium text-sm text-gray-800">{{ $brand->name }}</div>
                                                </div>
                                            </td>
                                            ...
                                            <td class="py-3 px-4 text-center">
                                                <div class="flex items-center justify-center space-x-2">
                                                    <a href="{{ route('admin.brands.edit', $brand->id) }}" class="text-primary-500 hover:text-primary-600 transition-colors" title="Edit">
                                                        <i data-lucide="edit" class="w-4 h-4"></i>
                                                    </a>
                                                    <a href="{{ route('admin.brands.show', $brand->id) }}" class="text-primary-500 hover:text-primary-600 transition-colors" title="View">
            ...
            <div class="flex items-start gap-4">
                ...
                <div class="flex-1">
                    ...
                    <p class="mt-2 text-sm text-gray-600">
                        Are you sure you want to delete <span id="brand-name-display" class="font-medium text-gray-800"></span>? This action cannot be undone.
                    </p>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        deleteButtons.forEach(button => {
            button.addEventListener('click', function() {
                deleteFormId = this.getAttribute('data-brand-id');
                const brandName = this.getAttribute('data-brand-name');
                brandNameDisplay.textContent = brandName;
