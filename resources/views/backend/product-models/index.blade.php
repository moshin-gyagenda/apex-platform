    ...
    <div class="p-4 sm:ml-64 mt-16 flex flex-col min-h-screen">
            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-3">
                ...
                <li aria-current="page">
                    <div class="flex items-center">
                        ...
                        <span class="text-sm font-medium text-gray-500">Product Models</span>
                    </div>
                </li>
            </ol>
        </nav>
        ...
        <div class="container mx-auto py-6 px-4 sm:px-6">
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                <h1 class="text-xl font-semibold text-gray-800">Product Model Management</h1>
                <div class="flex space-x-2">
                    ...
                    <a href="{{ route('admin.product-models.create') }}" class="inline-flex items-center px-4 py-2 bg-primary-500 border border-transparent rounded-lg text-sm font-medium text-white hover:bg-primary-600 transition-colors">
            ...
            <div class="bg-white rounded-lg border border-gray-200 p-4 mb-6">
                <form method="GET" action="{{ route('admin.product-models.index') }}" class="flex flex-col sm:flex-row gap-4">
                    <div class="flex-1">
                        <div class="relative">
                    ...
                    @if(request('search') || request('brand_id'))
                        <a
                            href="{{ route('admin.product-models.index') }}"
                ...
                <div id="product-models-content">
                    @if($productModels->isEmpty())
                        <div class="flex flex-col items-center justify-center py-12">
                            ...
                            <h3 class="text-lg font-medium text-gray-700">No product models found</h3>
                            <p class="mt-2 text-sm text-gray-500 text-center max-w-sm">
                                You don't have any product models yet. Add your first product model to get started.
                            </p>
                            <a href="{{ route('admin.product-models.create') }}" class="mt-4 inline-flex items-center px-4 py-2 bg-primary-500 text-white rounded-lg hover:bg-primary-600 transition-colors font-medium">
                                <i data-lucide="plus" class="w-4 h-4 mr-2"></i>
                    ...
                    @else
                            ...
                            <table class="w-full">
                                <thead>
                                    <tr class="border-b border-gray-200 bg-gray-50">
                                        ...
                                        <th class="py-3 px-4 text-left text-sm font-semibold text-gray-700 hidden lg:table-cell">Model Number</th>
                                ...
                                <tbody>
                                    @foreach($productModels as $index => $productModel)
                                        <tr class="border-b border-gray-100 hover:bg-gray-50 transition-colors {{ $index % 2 === 0 ? 'bg-white' : 'bg-gray-50/30' }}">
                                            <td class="py-3 px-4 text-sm text-gray-600">{{ $productModel->id }}</td>
                                            <td class="py-3 px-4">
                                                <div class="flex items-center">
                                                    ...
                                                    <div class="font-medium text-sm text-gray-800">{{ $productModel->name }}</div>
                                                </div>
                                            </td>
                                            ...
                                            <td class="py-3 px-4 text-center">
                                                <div class="flex items-center justify-center space-x-2">
                                                    <a href="{{ route('admin.product-models.edit', $productModel->id) }}" class="text-primary-500 hover:text-primary-600 transition-colors" title="Edit">
                                                        <i data-lucide="edit" class="w-4 h-4"></i>
                                                    </a>
                                                    <a href="{{ route('admin.product-models.show', $productModel->id) }}" class="text-primary-500 hover:text-primary-600 transition-colors" title="View">
                                                    ...
                                                    <button
                                                        ...
                                                        title="Delete"
                                                        data-product-model-id="{{ $productModel->id }}"
                                                        data-product-model-name="{{ $productModel->name }}">
                                                        <i data-lucide="trash-2" class="w-4 h-4"></i>
                                                    </button>
                                                    <form id="delete-form-{{ $productModel->id }}" action="{{ route('admin.product-models.destroy', $productModel->id) }}" method="POST" class="hidden">
                                                        @csrf
                                                        @method('DELETE')
                                                    </form>
                                                </div>
                        ...
                        <div class="mt-4 px-4 py-3 border-t border-gray-200">
                            {{ $productModels->appends(request()->query())->links() }}
                        </div>
                    @endif
                </div>
            </div>
    ...
    <div id="confirmation-popup" class="hidden fixed inset-0 bg-gray-500 bg-opacity-75 flex items-center justify-center z-50">
        <div class="bg-white rounded-lg border border-gray-300 shadow-xl p-6 max-w-md w-full mx-4">
            <div class="flex items-start gap-4">
                ...
                <div class="flex-1">
                    <h3 class="text-lg font-semibold text-gray-800">Delete Product Model</h3>
                    <p class="mt-2 text-sm text-gray-600">
                        Are you sure you want to delete <span id="product-model-name-display" class="font-medium text-gray-800"></span>? This action cannot be undone.
                    </p>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const productModelNameDisplay = document.getElementById('product-model-name-display');
        const confirmDeleteBtn = document.getElementById('confirm-delete');
        const cancelDeleteBtn = document.getElementById('cancel-delete');

        deleteButtons.forEach(button => {
            button.addEventListener('click', function() {
                deleteFormId = this.getAttribute('data-product-model-id');
                const productModelName = this.getAttribute('data-product-model-name');
                productModelNameDisplay.textContent = productModelName;
